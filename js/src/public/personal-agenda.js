document.addEventListener("DOMContentLoaded", () => {
    const schedules = document.querySelectorAll(".sympose-schedule.event.personal-agenda");
    let savedSessions = [];
        ...
        schedules.forEach((schedule) => {
            ...
            schedule.addEventListener("click", (e) => {
                ...
                if (e.target.closest(".saved-sessions-control")) {
                    ...
                    if (
                        schedule.querySelectorAll('.session-row[data-state="on"]').length > 0
                    ) {
                        schedule.dataset.showFavorites =
                            schedule.dataset.showFavorites === "true" ? "false" : "true";
                    }
                } else {
                    if (
                        schedule.closest(".sympose-schedule").classList.contains("updating")
                    ) {
                        return;
                    }
                    ...
                    if (target !== null) {
                        ...
                        if (
                            ...
                            target.parentNode.dataset.state === "off"
                        ) {
                            newState = "on";
                        }
    ...
    savedSessions.map((event) => {
        if (schedule.dataset.id === event.id) {
            event.sessions.map((session_id) => {
                let session = schedule.querySelector(
                    '.session-row[data-id="' + session_id + '"]'
                );
                if (session !== null) {
