...
class Sympose_Social_Media {
	...
	public function hooks() {
		...
		add_filter( 'sympose_organisation_after_content', array( $this, 'organisation_after_content' ), 10, 2 );
	...
	public function organisation_after_content( $content, $id ) {
		$social_media = get_post_meta( $id, '_sympose_social_media', true );

		if ( ! $social_media || empty( $social_media ) ) {
			return $content;
		}

		$content .= '<div class="sympose-social-media">';
		foreach ( $social_media as $item ) {
			if ( isset( $item['url'] ) && ! empty( $item['url'] ) ) {
				$content .= '<span class="sympose-social-media-icon"><a target="_blank" href="' . $item['url'] . '"><i class="fab fa-' . $item['fa'] . '"></i></a></li>';
			}
		}
		$content .= '</div>';
	...
	public function add_widget_content() {
		...
		if ( $social_media ) {
			...
			foreach ( $social_media as $item ) {
				if ( isset( $item['url'] ) && ! empty( $item['url'] ) ) {
					echo '<a target="_blank" href="' . $item['url'] . '">' . $this->render_social_media_icon( $item['fa'] ) . '</a>';
				}
			}
			?>
			</div>
